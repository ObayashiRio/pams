...
class DQNAgent(Agent):
    """A minimal DQN-like agent.

    This agent does not depend on external deep learning libraries. Instead, it
    keeps Q-values for a small discrete state space similar to
    :class:`QLearningAgent` and updates them with a pseudo DQN rule. This is not
    an efficient implementation but works as a placeholder when full deep
    learning frameworks are unavailable.
    """
    ...
    def _update_q(self, reward: float, state: int) -> None:
        ...
        self.q_table[prev_key] = prev_q + self.alpha * (target - prev_q)
